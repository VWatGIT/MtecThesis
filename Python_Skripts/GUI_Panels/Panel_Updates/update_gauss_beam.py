def update_gauss_beam(root):
    theta = float(root.new_measurement_panel.nametowidget("simulation_frame").nametowidget("alpha_entry").get())
    phi = float(root.new_measurement_panel.nametowidget("simulation_frame").nametowidget("beta_entry").get())
    ...
    root.log.log_event(f"Updated Gauss Beam Parameters: | theta: {theta} , phi: {phi} |")