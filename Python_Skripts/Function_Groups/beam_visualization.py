...
def create_slices(data, orientation='vertical'):
    ...
    if orientation == 'vertical':
        coord_index = 0 # x coordinate
    if orientation == 'horizontal':
        coord_index = 2 # z coordinate
...
def process_slices(root, data):
    try:
        ...
        for key in sorted_keys:
            ...
            analyze_slice_2D(slice, data) # heatmap for vertical slices created here
        ...
        root.log.log_event(f"Error creating slices: {e}")
    ...
    try:
        ...
        root.log.log_event(f"Error creating beam model: Multiple planes on the X axis needed")

    # TODO remove this, as trajectoy and angle calculation is now done after a center search
    try:
        ...
        root.log.log_event(f"Error calculating trajectory and angles:")    
    
    # Rotate the beam to align with the x-axis TODO remove this
    try:
        beam = data['Visualization']['Beam_Models']['Measured_Beam']
        beam = rotate_beam(beam)
        data['Visualization']['Beam_Models']['Rotated_Beam'] = beam
    except Exception as e:
        root.log.log_event(f"Error rotating beam")
